...
highWind <- train[train$WindSpeed >= 13, 4:9]

high_cor <- round(cor(highWind),2)


# Get lower triangle of the correlation matrix
high_lower_tri<-function(high_cor){
  high_cor[upper.tri(high_cor)] <- NA
  return(high_cor)
}
# Get upper triangle of the correlation matrix
high_upper_tri <- function(high_cor){
  high_cor[lower.tri(high_cor)]<- NA
  return(high_cor)
}

upper_tri <- high_upper_tri(high_cor)
